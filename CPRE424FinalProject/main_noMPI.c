...
const int MAX_SINGLE = 0;
const int MAX_DOUBLE = 1;
const int MAX_TRIPLE = 2;
...
int main() {
    ...
    for (int k = 0; k < ITER; k++) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < 14; j++) {
                printf("%d ", faceCount[i][j]);
            }
            printf("\n");
        }
         */

        //printAllHands(hands, handsFull, extraHand);
    ...
    printf("%10.2f%10.2f%10.2f%10.2f%10.2f\n", totalTally[TRIPLE] * 1.0 / ITER / 3.0, totalTally[QUARTET] * 1.0 / ITER / 3.0, totalTally[PENT_SINGLE] * 1.0 / ITER / 3.0, totalTally[TRI_DOUBLE] * 1.0 / ITER / 3.0, totalTally[DUAL_TRIPLE] * 1.0 / ITER / 3.0);

    printf("%10.2f%10.2f%10.2f%10.2f%10.2f\n", pTotalTally[TRIPLE] * 1.0 / ITER / 2.0, pTotalTally[QUARTET] * 1.0 / ITER / 2.0, pTotalTally[PENT_SINGLE] * 1.0 / ITER / 2.0, pTotalTally[TRI_DOUBLE] * 1.0 / ITER / 2.0, pTotalTally[DUAL_TRIPLE] * 1.0 / ITER / 2.0);
    printf("%10.2f%10.2f%10.2f%10.2f%10.2f\n", lTotalTally[TRIPLE] * 1.0 / ITER, lTotalTally[QUARTET] * 1.0 / ITER, lTotalTally[PENT_SINGLE] * 1.0 / ITER, lTotalTally[TRI_DOUBLE] * 1.0 / ITER, lTotalTally[DUAL_TRIPLE] * 1.0 / ITER);
...
//quartet: 4, triple 1, qs 1, td 1, dt 3
void processData(int hands[3][perHand], int handsFull[3][perHand + 3]) {
    //p1 as landlord
...
void addToTally(int l1, int p1, int p2, int hands[3][perHand], int handsFull[3][perHand + 3]) {
    for (int i = 0; i < numTally; i++){
        ...
        totalTally[i] += tally[p1 * 2][i] + tally[p2 * 2][i];
        totalTally[i] += tally[l1 * 2 + 1][i];
    }
}
    ...
    for(int i = 0; i < 6; i++) {
        ...
        for (int j = 0; j < faces - 4; j++) {
            if (faceCount[i][j] && faceCount[i][j + 1] && faceCount[i][j + 2] && faceCount[i][j + 3] && faceCount[i][j + 4] ) {
                ...
                int counter = j;
                int length = 5;
                while (counter < faces - 1 && faceCount[i][counter + 1] > 0) {
                    length++;
                    counter++;
                }

                if (length > maxLength[i][MAX_SINGLE]) {
                    maxLength[i][MAX_SINGLE] = length;
                }
